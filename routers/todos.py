...
async def create_todo(request: Request, title: str = Form(...), description: str = Form(...), priority: int = Form(...), db: Session = Depends(get_db)):
    ...
    todo_model.complete = False # Default value
...
@router.get('/delete-todo/{todo_id}', response_class=HTMLResponse) # questo perché non si può fare una richiesta di tipo DELETE da un form html, se no si può fare direttamente con un metodo delete!
